...
public class RoundsCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("rounds")
                ...
                .then(CommandManager.literal("upgrades")
                        .executes(context -> {
                            ...
                            if (!RoundsZero.GAME_MANAGER.isWaitingForUpgradeChoice(player)) {
                                player.sendMessage(Text.literal("Тебе сейчас не нужно выбирать улучшение.").formatted(Formatting.GRAY), false);
                                return 0;
                            }
                            ...
                            player.sendMessage(
                                    Text.literal("Выбери карту командой /rounds pick <1-5>").formatted(Formatting.GREEN),
                                    false
                            );
                ...
                .then(CommandManager.literal("pick")
                        .then(CommandManager.argument("slot", com.mojang.brigadier.arguments.IntegerArgumentType.integer(1, 5))
                                .executes(context -> {
                                    ServerPlayerEntity player = context.getSource().getPlayer();
