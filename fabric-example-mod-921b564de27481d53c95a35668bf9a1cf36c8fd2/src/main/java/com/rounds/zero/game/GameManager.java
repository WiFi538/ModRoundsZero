...
public class GameManager {
    ...
    public void handleShieldRequest(ServerPlayerEntity player) {
        if (!canUseCombatInput(player)) {
            return;
        }

        combatManager.tryActivateShield(player);
    }
    ...
    public void handleReloadRequest(ServerPlayerEntity player) {
        ...
        combatManager.tryStartReload(player);
    }
    ...
    private boolean canUseCombatInput(ServerPlayerEntity player) {
        if (gameState != GameState.ROUND_ACTIVE) {
            return false;
        }
    ...
    public void handlePlayerDeath(MinecraftServer server, ServerPlayerEntity player) {
        ...
        alivePlayers.remove(player.getUuid());
        combatManager.resetPlayerState(player);
    ...
    public void startNextRound(MinecraftServer server) {
        ...
        for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
            ...
            CombatStats computedStats = CombatStats.createDefault();
            combatManager.preparePlayerForRound(player, computedStats);
